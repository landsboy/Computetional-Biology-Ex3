...
# The main function - receives 2 files  wnet0 and testnet0 file and a prediction will be made for the samples:
if __name__ == '__main__':
    # First we will check that the file wnet0 exists:
    if not os.path.exists("wnet0.txt"):
    ...
    # We will build the network from the data we have in wnet0
    architecture, weights, biases = import_network("wnet0.txt")
    ...
    # We will make a prediction and record it in the file "predic_results0":
    predict(network, "predic_results0.txt" , test_data)